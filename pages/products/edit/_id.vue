        ...
        <div class="mx-auto w-full max-w-[550px]">
            ...
            <div class="mb-5">
                ...
                <select v-model="formProduct.colors" id="countries" multiple class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option :value="color" v-for="(color, index) of availableColors" :key="index" :selected="formProduct.colors.includes(color)">{{ color }}</option>
                </select>
            </div>
            <div class="mb-5">
                <label
            ...
            <div class="mb-5">
                ...
                <div class="mt-2">
                    <ul id="image-template" class="overflow-hidden" v-if="formProduct?.images_product.length > 0 && !isFileFilled">
                        <li class="block p-1 w-[100px] h-24 overflow-hidden">
                            ...
                            <img alt="upload preview" class="img-preview w-full h-full sticky object-cover rounded-md bg-fixed overflow-hidden" :src="`${originalApi}/storage/v1/object/public/${formProduct?.images_product[0]}`" />
                    ...
                    <ul id="image-template" class="hidden overflow-hidden" v-else>
                        <li class="block p-1 w-[100px] h-24 overflow-hidden">
                            <article tabindex="0" class="group hasImage w-full h-full rounded-md focus:outline-none focus:shadow-outline bg-gray-100 cursor-pointer relative text-transparent hover:text-white shadow-sm overflow-hidden">
                            <img alt="upload preview" class="img-preview w-full h-full sticky object-cover rounded-md bg-fixed overflow-hidden" :src="`${originalApi}/storage/v1/object/public/${formProduct?.images_product[0]}`" />
...
export default {
    async asyncData({isDev, route, store, env, params, query, req, res, redirect, error}) {
        ...
        const originalApi = env?.originalAPI;
    ...
    data() {
        return {
            availableColors: ['Red', 'Black', 'White', 'Brown'],
            formProduct: {
                name: '',
                description: '',
                colors: [],
    ...
    methods: {
        ...
        async upload() {
            try {
                ...
                this.formProduct.images_product = [this.image_product]
        ...
        setInitialForm() {
            this.formProduct.name = this.product?.name || ''
            this.formProduct.description = this.product?.description || ''
            this.formProduct.colors = this.product?.colors || []
            this.formProduct.price = this.product?.price || 0
            this.formProduct.images_product = this.product?.images_product || 0
        },
    }
}
</script>
